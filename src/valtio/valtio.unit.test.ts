...
describe("createHandler seam: throwing defineProperty/setPrototypeOf traps", () => {
			...
			const replacement: CreateHandler = (isInitializing, addPropListener, removePropListener, notifyUpdate) => {
				...
				return {
					...
					defineProperty(target, prop, descriptor) {
						...
						throw new Error(
							"opshot: defineProperty is not supported on tracked state; define properties in the createMutableState literal",
						);
					},
					setPrototypeOf() {
	...
	it("throws the added error on a consumer Object.defineProperty", () => {
		...
		expect(() => Object.defineProperty(state, "y", { value: 1 })).toThrow(
			"opshot: defineProperty is not supported on tracked state",
		);
		expect(setDepth).toBe(0);
	});
...
describe("facade and identity probes", () => {
	...
	it("seeds a copy registry for every nested snapshot copy during the snapshot walk", () => {
		...
		const createRegisteredSnapshot = <T extends object>(target: T, version: number): T => {
			...
			const copy: object = Array.isArray(target)
				? []
				: Object.create(Reflect.getPrototypeOf(target) as object | null);
		...
		if (!rootState || !outerState || !innerState)
			throw new Error("registry probe: expected proxy state at every level");
